...
from tasks import process_signed_document
from workflow import get_workflow


def register(config: LHConfig) -> None:
    littlehorse.create_task_def(
        process_signed_document, "processSignedDocument", config
    )
...
async def workers(config: LHConfig) -> None:
    await littlehorse.start(
        LHTaskWorker(process_signed_document, "processSignedDocument", config)
    )
