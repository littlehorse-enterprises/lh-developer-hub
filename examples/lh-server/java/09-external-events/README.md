...
> **Note:** This runnable uses `.registeredAs(String.class)` to register the typed `ExternalEventDef` when the `WfSpec` is registered. The website snippet registers the `ExternalEventDef` programmatically with `putExternalEventDef()`. Both approaches are supported.
...
lhctl postEvent <wf-run-id> name-posted STR Obi-Wan
```